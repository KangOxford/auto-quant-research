...
9. **Export PNG**: `rsvg-convert -w 2400 file.svg -o file.png`
...
- ViewBox: `0 0 720 480` standard, `0 0 720 540` for tall stacks
...
- Align nodes on a grid: x positions snap to 100px intervals, y to 70px
...
- ViewBox width = 720; height = 80 + (num_messages × 45)
...
- ViewBox: `0 0 720 360` typical; wider for many time periods: `0 0 900 360`

### Mind Map / Concept Map
Radial layout from central concept.
- Central node at `cx=360, cy=240`
...
| Control / trigger | orange `#ea580c` | 2px solid | none | One system triggering another |
| Memory read | green `#059669` | 2px solid | none | Retrieval from store |
| Memory write | green `#059669` | 2px | `5,3` | Write/store operation |
| Async / event | gray `#6b7280` | 2px | `4,2` | Non-blocking, event-driven |
| Embedding / transform | purple `#7c3aed` | 1.5px solid | none | Data transformation |
| Feedback / loop | purple `#7c3aed` | 2px curved | none | Iterative reasoning loop |
...
**Spacing**:
- Same-layer nodes: 60px horizontal, 90px vertical between layers
- Canvas margins: 30px minimum, 50px between node edges
- Snap to 8px grid: horizontal 100px intervals, vertical 90px intervals
...
- ViewBox: `0 0 720 480` default; `0 0 720 540` tall; `0 0 900 480` wide
...
- Text: minimum 13px, prefer 16px labels, 14px sub-labels, 20-22px titles
- All arrows: `<marker>` with `markerEnd`, sized `markerWidth="12" markerHeight="8"`
- Arrow stroke: `stroke-width="2"` (was 1.5; thicker for print readability)
...
### Paper Figure Scale Reference

When a diagram is placed in a LaTeX paper at `\textwidth` (~6.5 inches), the effective pt size = SVG_px × (6.5 × 72 / viewBox_width). For viewBox width 720:

| SVG px | Paper pt | Use for |
|--------|----------|---------|
| 22px   | 14.3pt   | Diagram title |
| 20px   | 13.0pt   | Section/module titles |
| 16px   | 10.4pt   | Labels (matches 10pt body text) |
| 14px   | 9.1pt    | Sub-labels, annotations |
| 13px   | 8.5pt    | Minimum readable text |

For `\columnwidth` (~3.3 inches, two-column papers), use viewBox `0 0 400 300` with the same font sizes to maintain readability.
...
python3 << 'EOF'
lines = []
lines.append('<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 720 480">')
lines.append('  <defs>')
...
- **PNG export**: `rsvg-convert -w 2400 file.svg -o file.png` (2400px, ~370dpi at textwidth for print quality)
